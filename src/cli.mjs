...
import { presetList } from './presets.mjs';
import { chooseCreateMode, pickBuiltinPreset, providerFormWithPreview } from './form.mjs';
import { launch, dryRun, launchDefault, dryRunDefault, buildProviderSettings, redactSettings } from './launch.mjs';
import { ui, Cancel } from './tui.mjs';
import { t, detectLocale, setConfig, LOCALES } from './i18n.mjs';
...
async function main() {
  ...
  try {
    ...
    switch (cmd) {
      ...
      case 'create': return await cmdCreate(rest);
...
function cmdLaunchDefault(rest) {
  const dry = rest.includes('--dry-run');
  const forwarded = rest.filter((a) => a !== '--dry-run');
...
async function pickExistingProvider(message) {
  ...
  return ui.select({ message, options, initialValue: options[0].value });
}
...
async function cmdUse(rest) {
  ...
  for (;;) {
    ...
    const options = names.map((n) => ({
      value: { kind: 'provider', name: n },
      label: n,
      hint: n === last ? t('list.lastUsed') : '',
    }));
    options.push(
      { value: { kind: 'default' }, label: t('use.default'), hint: t('use.defaultHint') },
      { value: { kind: 'create' }, label: t('use.create'), hint: t('use.createHint') },
    );
    if (names.length) {
      options.push(
        { value: { kind: 'edit' }, label: t('use.edit'), hint: t('use.editHint') },
        { value: { kind: 'remove' }, label: t('use.remove'), hint: t('use.removeHint') },
      );
    }
    ...
    const picked = await ui.select({ message: t('use.select'), options, initialValue });
