...
const CCS_ONLY_KEYS = ['dangerouslySkipPermissions'] as const;

export function stripCcsKeys(obj: ProviderSettings): ProviderSettings {
  if (!obj || typeof obj !== 'object') return obj;
  const next: ProviderSettings = { ...obj };
  for (const k of CCS_ONLY_KEYS) {
    delete next[k];
  }
  return next;
}
...
export function readCcsFlags(name: string): CcsFlags {
  const provider = readProvider<ProviderSettings>(name) || { env: {} };
  return {
    dangerouslySkipPermissions: provider.dangerouslySkipPermissions === true,
  };
}

/** 根据 ccs 启动开关构造要透传给 claude 的 CLI 参数。 */
export function ccsFlagArgs(flags: CcsFlags): string[] {
  const args: string[] = [];
 ...
 */
export function buildProviderSettings(name: string): ProviderSettings {
...
/** 向后兼容旧名。等价于 buildProviderSettings。 */
 ...
 */
export function mergedPath(name: string): string {
